...
The dataset used is the Boston Marathon 2017 results. It contains information on all finishers including their name, age, gender, origin, split times at every 5K mark, pace, and official finishing time.
...
{"columns":[{"label":["Total_Runners"],"name":[1],"type":["int"],"align":["right"]},{"label":["Countries"],"name":[2],"type":["int"],"align":["right"]},{"label":["Median_Age"],"name":[3],"type":["dbl"],"align":["right"]},{"label":["Median_Time"],"name":[4],"type":["dbl"],"align":["right"]},{"label":["Female_Pct"],"name":[5],"type":["dbl"],"align":["right"]}],"data":[{"1":"26410","2":"91","3":"43","4":"231.6583","5":"45.33131"}],"options":{"columns":{"min":{},"max":[10]},"rows":{"min":[10],"max":[10]},"pages":{}}}
  </script>
</div>
...
ggplot(world_sf) +
  ...
  labs(
    ...
    subtitle = "Green = faster than average, orange-red = slower. White = no participants."
  ) +
  theme_minimal() +
  theme(
...
<img src="siedler_project_02_files/figure-html/spatial-choropleth-1.png" alt="World choropleth map showing average Boston Marathon 2017 finishing time by country. Green countries are faster-than-average, red countries slower. The USA appears in a neutral grey tone near the global average. Most of Africa and parts of Asia have no participants and are shown in white."  />
...
Average official finishing time at each individual age (18 to 70), split by gender. Performance peaks in the early 20s to mid-30s, then gradually declines. The gender gap is consistent at about 30 minutes across all ages. Colors follow the Okabe-Ito colorblind-safe palette.
...
I'm interested in running, which led me to pick the Boston Marathon 2017 dataset.

### Original Plans and Data Preparation

- I initially planned to visualize participant counts per country on a map, but the US dominated the dataset so heavily that it was not informative. Even a log scale did not help.
- I wanted to explore whether US runners aim for "round" mile-based pace targets (e.g., 8:00/mile) vs. km-based ones, but participants focus more on total target times than pace splits.
- An interesting future direction: animate runners on the actual race route using the 5K, 10K, … split times to interpolate positions over time.

### What the Plots Reveal

The interactive histograms show an age peak around 40 and a finish time peak in the 3.5–4.5 hour range. The finish time histogram shows dramatic spikes just before 3h and 4h — the psychological "holy grail" barriers. The choropleth shows international runners are faster on average, consistent with the selection effect of traveling to compete. The age/gender plot confirms performance peaks in the late 20s to mid-30s with a ~30-minute consistent gender gap. The linear model quantifies: +0.56 min/year of age, +28 min for female vs. male.

### Design Principles Applied

`theme_bw()` is used throughout for consistent, high-contrast styling. Colors follow the Okabe-Ito palette (`#0072B2`, `#D55E00`, `#009E73`) which is safe for all major colorblindness types. The choropleth uses a diverging scale centered on the global average, making above/below-average immediately apparent. Annotations are placed directly on the canvas to reduce reliance on legends. The interactive histograms use tooltip-based information so the chart canvas stays uncluttered.
